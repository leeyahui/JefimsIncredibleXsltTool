...
namespace XsltTool
{
    ...
    public partial class MainWindow
    {
        ...
        private void ChangeColorTheme(ColorTheme colorTheme)
        {
            ...
            Settings.Default.theme = colorTheme.Id;
        ...
        private void Window_Closing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            {
                Settings.Default.xmlContent = _mainViewModel.XmlToTransformDocument.Text;
            }
            if (_mainViewModel.Document != null)
            {
                Settings.Default.xsltPath = _mainViewModel.Document.FilePath;
                Settings.Default.xsltContentIfPathEmpty = _mainViewModel.Document.TextDocument.Text;
            }

            if (_mainViewModel.ValidationSchemaFile != null)
            {
                Settings.Default.xsdPath = _mainViewModel.ValidationSchemaFile;
            }
        ...
        private void LoadOldSettings()
        {
            try
            {
                if (Settings.Default.xsltPath != null && File.Exists(Settings.Default.xsltPath))
                {
                    _mainViewModel.OpenFile(Settings.Default.xsltPath);
                }
                else if (Settings.Default.xsltContentIfPathEmpty != null)
                {
                    _mainViewModel.Document.TextDocument.Text = Settings.Default.xsltContentIfPathEmpty;
                }

                if (Settings.Default.xmlContent != null)
                {
                    _mainViewModel.XmlToTransformDocument.Text = Settings.Default.xmlContent;
                }
                if (Settings.Default.xsdPath != null && File.Exists(Settings.Default.xsdPath))
                {
                    _mainViewModel.ValidationSchemaFile = Settings.Default.xsdPath;
                }

                var theme = ColorTheme.ColorThemes.FirstOrDefault(t => t.Id == Settings.Default.theme);
